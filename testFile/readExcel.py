...
class readExcel():
    def get_xls(self, xls_name, sheet_name):
        ...
        for i in range(nrows):  # 根据行数做循环
            if sheet.row_values(i)[0] != u'case_name': # 如果这个Excel的这个sheet的第i行的第一列不等于case_name，那么我们把这行的数据添加到cls[]中
                cls.append(sheet.row_values(i))
        return cls


if __name__ == '__main__': # 我们执行该文件测试一下是否可以正确获取Excel中的值
    print(readExcel().get_xls('userCase.xlsx', 'creatreceiptOrder'))
    print(readExcel().get_xls('userCase.xlsx', 'creatreceiptOrder')[0][1])
